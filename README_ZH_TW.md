...
- [Hello world on Renec](#hello-world-on-renec)
  ...
  - [學習 Client](#學習-client)
    ...
    - [查詢使用過 `Hello` 交易的 Renec 帳戶](#查詢使用過-Hello-交易的-Renec-帳戶)
  - [學習鏈上程式](#學習鏈上程式)
    - [在 Renec 上編寫程式](#在-Renec-上編寫程式)
  - [指向公開 Renec 集群](#指向公開的-Renec-集群)
  - [透過高級的範例擴展你的技能](#透過高級的範例擴展你的技能)

## 快速開始

[![Open in Gitpod](https://gitpod.io/button/open-in-gitpod.svg)](https://gitpod.io/#https://github.com/renec-labs/example-helloworld)
...
- 從 https://docs.renec.com/cli/install-renec-cli-tools 安裝 v1.8.2+ 的 Renec 命令列管理工具
...
如果這是你第一次使用 renec 命令列，你先得生成一個新的密鑰對
...
默認情況下，此範例連接到本地 Renec 集群。

啟動本地 Renec 集群：
```bash
$ renec-test-validator
```
**注意: 如果你要用 Windows 的話，你得先設置 WSL，才能用 `renec-test-validator` 的工具**
...
- 確認您已經[啟動了本地 Renec 集群](#start-local-renec-cluster)，[構建](#構建鏈上程式) 並 [部署好了](#部署鏈上程式) 鏈上程式。
- 集群的輸出日誌應包括程序日誌消息以及程式失敗的原因
  - `program log: <message>`   
- 運行 `renec logs` 檢查程式日誌找出程式失敗的原因。
  - ```bash
...
[Renec 文件](https://docs.renec.com/)提供了有關 Renec 的更多消息並且所有的源代碼都在 [github](https://github.com/renec-labs/renec) 上。
...
[客戶端入口點](https://github.com/renec-labs/example-helloworld/blob/e936ab42e168f1939df0164d5996adf9ca635bd0/src/client/main.js#L14)做了四件事

### 建立與集群的連接

客戶端通過調用 [`establishConnection`](https://github.com/renec-labs/example-helloworld/blob/e936ab42e168f1939df0164d5996adf9ca635bd0/src/client/hello_world.js#L45) 與客戶端建立連接.
...
客戶端將通過調用 [`sayHello`](https://github.com/renec-labs/example-helloworld/blob/e936ab42e168f1939df0164d5996adf9ca635bd0/src/client/hello_world.js#L121) 並向程式發送 `Hello` 交易。此交易包含一條非常簡單的指令，此指令主要呼叫 `helloworld` 程式的帳戶公鑰希望向 `greeter` 帳戶說 `Hello`。

### 查詢使用過 `Hello` 交易的 Renec 帳戶

客戶端每次對帳戶說 `Hello` 時，程式都會在 `greeter` 帳戶的數據中增加一個計數。客戶端查詢 `greeter` 帳戶的數據，並透過 [`reportHellos`](https://github.com/renec-labs/example-helloworld/blob/e936ab42e168f1939df0164d5996adf9ca635bd0/src/client/hello_world.js#L138.) 查詢此帳戶當前被訪問的次數。
...
要了解有關 Renec 程式設計模型的更多訊息，請參閱[程式設計模型概述](https://docs.renec.com/developing/programming-model/overview)。

要了解有關在 Renec 上開發程式的更多訊息，請參閱[已部署程式概述](https://docs.renec.com/developing/deployed-programs/overview)。
